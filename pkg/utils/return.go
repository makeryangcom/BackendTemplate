...
func requestLog(c *gin.Context, status string, response string) {
	...
	clientTimeStr := c.Request.Header.Get("content-x-time")
	if clientTimeStr == "" {
	...
	log.Println(
		...
		color.Gray.Text(fmt.Sprintf("%-19s⇒", "")), c.Request.Header.Get("content-x-source"), c.ClientIP(), c.Request.Header.Get("content-x-ip"), c.Request.Header.Get("content-x-device"), "\n",
		icon,
		response)
}
...
func Header(c *gin.Context, code int, message string, data string) {

	c.Header("X-Auth-Token", data)
	c.Header("X-Auth-Message", message)
	c.Status(code)
